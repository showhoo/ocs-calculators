...
- **型号代码规范化**： 与铜计算器的型号键由 H 前置误写形态
  （ /  / ）统一为站内规范写法 ** /  / **
  （H 后置构词，与 TB/T 2809-2026 牌号体系及站点词条口径一致）。
  ⚠️ 破坏性变更：以旧键字符串调用 copper 相关函数的调用方需同步改名，
  旧键传入会得到「未知型号」错误。
- 同日已在站点部署侧（itswe  与 7 个计算器页）
  完成相同归一，并修复 7 页输入控件的畸形 uid=197609(showhoo) gid=197609 groups=197609 属性（曾致 JS 绑定整体失效）。
